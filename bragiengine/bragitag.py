...
class BragitagEngine:
    ...
    def edit_filename(self, track_id, new_name):
        ...
        parent_path = os.path.abspath(os.path.join(
            self.metadata[track_id].filename, os.pardir))
        os.rename(self.metadata[track_id].filename,
                  os.path.join(parent_path, new_name) + ext[1])
        self.metadata[track_id] = music.load_file(
            os.path.join(parent_path, new_name) + ext[1])
